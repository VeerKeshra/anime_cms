...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($title_err) && empty($episode_number_err) && empty($air_date_err) && empty($synopsis_err) && empty($anime_id_err)) {
        $sql = "UPDATE Episodes SET title = :title, episode_number = :episode_number, air_date = :air_date, synopsis = :synopsis, anime_id = :anime_id WHERE episode_id = :id";
    ...
    if ($id) {
        $sql = "SELECT * FROM Episodes WHERE episode_id = :id";
        if ($stmt = $pdo->prepare($sql)) {
            $stmt->bindParam(":id", $param_id, PDO::PARAM_INT);
            $param_id = $id;
...
<body>
    <div class="wrapper">
        <h2>Edit Episode</h2>
        <p>Please fill this form to update the episode details.</p>
